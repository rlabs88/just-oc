...
describe("command_run observability", () => {
  test("requires a bounded primitive child inventory for stock generic clients", () => {
    const definition = createCommandRunTool({} as never)
    const description = definition.args.description as unknown as {
      safeParse(input: unknown): { success: boolean }
    }

    expect(description.safeParse("#0 step 1 read: package.json; #1 step 1 glob: plugins/**/*.ts").success).toBe(true)
