...
const COMMAND_GRAMMAR = `Run 1–20 permission-gated commands behind positive-integer dependency steps. description is a concise single-line summary of every submitted child in original order, grouped by dependency step. Use human-readable entries like "Step 1 · Read package.json · Glob plugins/**/*.ts → Step 2 · Shell bun test". Include each command_type and its target, omit child indices and raw JSON syntax, use · within a step, and use → between steps so stock TUI and GUI generic tool rows expose a readable batch. Each command may set timeout_ms from 100 to 300000 milliseconds; the default is 120000 and begins only when execution starts. Put every currently needed command whose input is already known into one batch. Independent read, search, list, and task-state commands must share a step; commands that depend on earlier output use a later step. Do not add a probe whose input depends on another command in the same batch. Commands in a later step wait for every command in earlier steps. command_line grammar by type: read JSON {"path":string,"offset"?:integer,"limit"?:integer}; glob JSON {"pattern":string,"path"?:string}; grep JSON {"pattern":string,"path"?:string,"include"?:string}; apply_patch raw unified diff; shell raw foreground shell command; task_status JSON {"task_group":string,"task_type":["implementation"|"new_build"|"debugging"|"refactoring"|"architecture"|"review"|"research"|"data_research"|"visual_inspection"|"frontend"|"website"|"interactive_3d"|"editorial"|"operations"],"status":"doing"|"question"|"done","compact_context"?:string}; web_discover JSON {"url":http-or-https-url,"mode":"extract"} or {"url":http-or-https-url,"mode":"download","path":workspace-relative-path}; read_media JSON {"path":string,"offset"?:integer,"limit"?:integer}.`

export function createCommandRunTool(client: PluginInput["client"]): ToolDefinition {
  return tool({
    description: COMMAND_GRAMMAR,
    args: {
      description: tool.schema.string().min(1).max(4_000).regex(/^[^\r\n]+$/, "description must be one line").describe(
        "Readable one-line child-command summary rendered by stock clients; group every type and target by dependency step."
      ),
